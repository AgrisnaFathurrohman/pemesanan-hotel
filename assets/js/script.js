...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Sistem Pemesanan Hotel Griya Karsa Dimulai');
    setMinDates();
    setupEventListeners();
});
...
function openBookingModal(roomName, roomPrice, roomCapacity) {
    ...
    modalRoomInfo.innerHTML = `
        <h3>${roomName}</h3>
        <p><strong>Harga:</strong> ${formatCurrency(roomPrice)} per malam</p>
        <p><strong>Kapasitas Maksimum:</strong> ${roomCapacity} tamu</p>
    `;
...
function showConfirmation(booking) {
    ...
    confirmationDetails.innerHTML = `
        <div style="text-align: left; margin: 1rem 0;">
            <p><strong>Referensi Pemesanan:</strong> MN${Date.now().toString().slice(-8)}</p>
            <p><strong>Nama Tamu:</strong> ${booking.fullname}</p>
            <p><strong>Email:</strong> ${booking.email}</p>
            <p><strong>Telepon:</strong> ${booking.phone}</p>
            <hr style="margin: 1rem 0; border: none; border-top: 1px solid #dfe6e9;">
            ...
            <p style="font-size: 1.2rem; color: #2c3e50;"><strong>Jumlah Total:</strong> ${formatCurrency(booking.total)}</p>
        </div>
    `;
